...
class Parted_Disk_init(Init_Base):
    ...
    def __init__(self,db_server_info,host_ip,ssh_con):
        super(Parted_Disk_init, self).__init__(db_server_info)
    ...
    def read_block_info(self):
        ...
        result,error = self.ssh_con.do_remote_by_passwd_exec(lvm_info_command)
        ...
        lvm_info[self.host_ip] = blocks
        partition_info = []
        blocks = {}

        result,error = self.ssh_con.do_remote_by_passwd_exec(pv_info_command)            
        if result == "wrong":
            logger.write_log("host: %s . read pv info failed." % self.host_ip)
            return False
    ...
    def get_search_ted_info(self):
        ...
        result = super(Parted_Disk_init, self).select_with_desc(sql,self.host_ip,self.host_ip)
        if len(result) > 0:
            for data in result:
        ...
        if len(partition_dic['opt']['partition_mount_point']) > 0 and len(partition_dic['ted']['partition_mount_point']) > 0:
            ...
            result,error = self.ssh_con.do_remote_by_passwd_exec(ted_subdir_exist_cmd)
            
            if len(result) > 0:
                logger.write_log("host: %s /search/ted is not NULL" % self.host_ip)
                return False
            ...
            if len(partition_dic['opt']['partition_type']) > 0:
                if  ext_rule.match(partition_dic['opt']['partition_type']):
                    ...
                    logger.write_log("host: %s. FS is not correct!" % self.host_ip)
                    return  False
            ...
            self.ssh_con.do_remote_by_passwd_exec(remove_ted_cmd)
...
class Exec_partition_cls(Init_Base):
    ...
    '''
    def __init__(self,db_server_info,host_ip,ssh_con):
        super(Exec_partition_cls, self).__init__(db_server_info)
        self.host_ip = host_ip
        self.ssh_con = ssh_con
    ...
    def set_vm_partition_info(self):
        ...
        result  =  super(Exec_partition_cls, self).select_with_desc(sql,self.host_ip,self.host_ip)
        ...
        if not exsit_data_partition:
            if not exsit_search_partition:
                result,error = self.ssh_con.do_remote_by_passwd_exec(new_partition_cmd)
                if result == "wrong":
                    logger.write_log("host: %s partition failed" % self.host_ip)
                    return False
                else:
                    return True 
                ...
                result,error = self.ssh_con.do_remote_by_passwd_exec(replace_partition_cmd)
                if result == "wrong":
                    logger.write_log("host: %s partition failed" % self.host_ip)
                    return False
                else:
            ...
            logger.write_log("host: %s exsit_data_partition" % self.host_ip)


    def partition_new(self,mount_dir,mount_type,mount_path):
        '''
            划分新分区信息
        '''
    ...
    def partition_rename(self,new_mount_dir,new_mount_type,new_mount_path):
        '''
            重新挂载分区
        '''
    ...
    def clean_partion_old(self,old_mount_dir,old_mount_path):
        '''
            清理老的分区
        '''
        ...
        old_mylog_flag = "LABEL=mylog*"

        new_mount_path = re.sub(r"/",r"\/",old_mount_path) 
        cmd = "fuser -ck %(mount_path)s;umount %(old_mount_dir)s;sed -i '/%(new_mount_path)s/d' /etc/fstab;sed -i '/%(old_data_dir_flag)s/d' /etc/fstab;sed -i '/%(old_mylog_flag)s/d' /etc/fstab;/bin/cp /etc/fstab /tmp/fstab.%(str_sn)s" % {'mount_path':old_mount_path,'str_sn':str_sn,'new_mount_path':new_mount_path,'old_mount_dir':old_mount_dir,'old_data_dir_flag':old_data_dir_flag,'old_mylog_flag':old_mylog_flag}
    ...
    def get_partition_data_by_ssh(self):
        '''
            获取分区信息以及大小
        '''
        ...
        ssh_result =  self.ssh_con.do_remote_by_passwd_exec(get_partition_cmd)        
        buf = StringIO.StringIO(ssh_result[0])
    ...
    def check_partition(self,mount_data_list):
        '''
            检查挂载目录，挂载类型，重新挂载分区
        '''
        db_mount_info_dic = {}
        ...
        db_mount_result = super(Exec_partition_cls, self).select_with_desc(sql,self.host_ip,self.host_ip)
                ...
                if mount_key in db_mount_info_dic:
                    ...
                    if mount_value['size'] == db_mount_info_dic[mount_key]['old_size']:
                            ...
                            if mount_value['relation'] == 'child':
                                ...
                                if mount_value['mount_dir'] == db_mount_info_dic[mount_key]['old_mount_dir']:
                                    ...
                                    if mount_value['mount_file_type'] == db_mount_info_dic[mount_key]['old_mount_file_type']:
                                        ...
                                        #重新按照标准格式化分区
                                        self.clean_partion_old(db_mount_info_dic[mount_key]['old_mount_dir'],mount_key)
                                        self.partition_new(mount_value['mount_dir'],mount_value['mount_file_type'],mount_key)
                                else:
                                    ...
                                    if mount_value['mount_file_type'] == db_mount_info_dic[mount_key]['old_mount_file_type']:
                                        #重命名目录
                                        self.partition_rename(mount_value['mount_dir'],mount_value['mount_file_type'],mount_key)
                                    else:
                                        #重新按照标准格式化分区
                                        self.partition_new(mount_value['mount_dir'],mount_value['mount_file_type'],mount_key)
                            else:
                        ...
                        ##没有子分区/dev/sdb1,sdc1等
                            ...
                            self.clean_partion_old(db_mount_info_dic[mount_key]['old_mount_dir'],mount_key)
                            #子分区命名为sdc1
                            self.do_parted(mount_key,'1')
                            new_mount_path = mount_key + '1'
                            self.partition_new(mount_value['mount_dir'],mount_value['mount_file_type'],new_mount_path)
                            break
                    ...
                    #分区大小不一致
                        ...
                        if mount_value['relation'] == 'parent': 
                            ...
                            #子分区命名为sdc1
                            self.do_parted(new_mount_key,'1')
                            self.partition_new(mount_value['mount_dir'],mount_value['mount_file_type'],mount_key)
                            break
                    #卸载用
        ...
        for key,data in db_mount_info_dic.items():
            self.clean_partion_old(data['old_mount_dir'],key)


    def set_pm_partition_info(self,package_type):
        '''
            处理实体机分区信息
        '''
        ...
        temp_size = 0
        machine_partition_list,machine_partition_dic = self.get_partition_data_by_ssh()
        ...
        for group_m_key,group_m_value in temp_disk_path_dic.items():
            ...
            group_dic = {}
        #分区      
        self.check_partition(mount_info_list)    
