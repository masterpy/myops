...
class Exec_partition_cls(Init_Base):
    ...
    def partition_new(self,mount_dir,mount_type,mount_path):
        ...
        if mount_type == 'xfs':
            format_str = "mkfs.xfs %s" % mount_path
        elif mount_type == 'ext3':
            format_str = "mkfs.ext3 %s" % mount_path
        elif mount_type == 'ext4':
            format_str = "mkfs.ext4 %s" % mount_path
    ...
    def rename_partion_old(self,mount_dir,new_mount_type,mount_path):
        '''
            重新挂载分区
        '''
        cmd  = "mkdir -p %(new_mount_dir)s;echo \"%(path)s %(new_mount_dir)s  xfs   defaults,noatime,nodiratime  0 0\" >> /etc/fstab" % {'new_mount_dir':mount_dir,'path':mount_path}
    ...
    def clean_partion_old(self,old_mount_dir,mount_path):
        '''
            清理老的分区
        '''
        import string
        ...
        new_mount_path = re.sub(r"/",r"\/",mount_path) 

        cmd = "fuser -ck %(mount_path)s;umount %(old_mount_dir)s;sed -i '/%(new_mount_path)s/d' /etc/fstab;/bin/cp /etc/fstab /tmp/fstab.%(str_sn)s" % {'mount_path':mount_path,'str_sn':str_sn,'new_mount_path':new_mount_path,'old_mount_dir':old_mount_dir}
            ...
            if len(result) > 0:
                ...
                for disk_info in result:
                    if disk_info['data_raid_info'] == 'raid-5':
                        ...
                        total_disk_data = int(disk_info['other_one_disk_size'][:-1]) * int(disk_info['other_disk_num']) / 2
                ...
                for disk_info in result:
                    for path,size in tempdic.items():
                        compare_data = (int(size[:-1]) > ( total_disk_data - 350)) and (int(size[:-1]) < (  total_disk_data + 350))

                        if compare_data:
                            tempdic2['data'] = {'mount_dir':disk_info['data_mount_dir'],'mount_type':disk_info['data_mount_dir_file_type'],'path':path}
  
                        else :                  
                            tempdic2['other'] = {'mount_dir':disk_info['other_mount_dir'],'mount_type':disk_info['other_mount_dir_file_type'],'path':path}
